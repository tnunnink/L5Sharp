...
public static class Extensions
{
    ...
    public static CliApplicationBuilder AddLogixServices(this CliApplicationBuilder builder)
    {
        return builder.UseTypeActivator(_ =>
        {
            ...
            services.AddKeyedSingleton<IElementRenderer, TableRenderer>(Format.Table);
            services.AddKeyedSingleton<IElementRenderer, JsonRenderer>(Format.Json);
            services.AddKeyedSingleton<IElementRenderer, CsvRenderer>(Format.Csv);
            services.AddKeyedSingleton<IElementRenderer, XmlRenderer>(Format.Xml);

            // Register projection services
            services.AddSingleton<IElementSchema<DataType>, DataTypeSchema>();
            services.AddSingleton<IElementSchema<Tag>, TagSchema>();
    ...
    /// <returns></returns>
    internal static IEnumerable<dynamic> SelectBy<T>(this IEnumerable<T> collection, string? properties)
    {
        if (string.IsNullOrEmpty(properties))
            return collection.ToDynamicList();

        return collection.AsQueryable().Select(properties).ToDynamicList();
    }
}