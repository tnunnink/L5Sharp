...
/// <summary>
/// Represents a <b>TIME32</b> or relative 32-bit time-based value measured in microseconds.
/// This atomic uses a <see cref="int"/> as the underlying primitive in .NET.
/// </summary>
[LogixData(nameof(TIME32), true)]
public sealed class TIME32 : AtomicData, IComparable, IConvertible, IAtomicValue<int>
{
    /// <inheritdoc />
    public TIME32(XElement element) : base(element)
    {
    }

    /// <summary>
    /// Creates a new default <see cref="TIME32"/> type.
    /// </summary>
    public TIME32() : base(nameof(TIME32))
    {
    }

    /// <summary>
    /// Creates a new <see cref="TIME32"/> with the provided value.
    /// </summary>
    /// <param name="value">The value to initialize the type with.</param>
    public TIME32(int value) : this()
    {
    ...
    public int CompareTo(object? obj)
    {
        ...
        {
            null => 1,
            TIME32 typed => Value.CompareTo(typed.Value),
    ...
    public override bool Equals(object? obj)
    {
        return obj switch
        {
            TIME32 value => Value == value.Value,
            AtomicData atomic => Value.Equals((int)Convert.ChangeType(atomic, typeof(int))),
            ValueType value => Value.Equals((int)Convert.ChangeType(value, typeof(int))),
            _ => false
        };
    }
    ...
    /// <summary>
    /// Parses the specified string representation of a <see cref="TIME32"/> value into its corresponding object.
    /// </summary>
    /// <param name="value">The string representation of the <see cref="TIME32"/> value to parse.</param>
    /// <returns>A <see cref="TIME32"/> object that represents the parsed value.</returns>
    public new static TIME32 Parse(string value)
    {
        var radix = Radix.Infer(value);
        var typed = radix.Parse<int>(value);
        var formatted = radix.Format(typed);
        return new TIME32(CreateDataElement(nameof(TIME32), radix, formatted));
    }

    /// <summary>
    /// Attempts to parse the specified string representation of a <see cref="TIME32"/> value and returns a value
    /// indicating whether the conversion succeeded.
    /// </summary>
    /// <param name="value">The string representation of the <see cref="TIME32"/> value to parse.</param>
    /// <param name="atomic">
    /// When this method returns, contains the <see cref="TIME32"/> equivalent of the parsed value if the conversion succeeded;
    /// otherwise, it is null.
    /// </param>
    /// <returns>
    /// <c>true</c> if the value was parsed successfully; otherwise, <c>false</c>.
    /// </returns>
    public static bool TryParse(string? value, out TIME32 atomic)
    {
        atomic = null!;
        ...
        if (Radix.TryInfer(value, out var radix))
        {
            var typed = radix.Parse<int>(value);
            var formatted = radix.Format(typed);
            atomic = new TIME32(CreateDataElement(nameof(TIME32), radix, formatted));
            return true;
        }
    ...
    public static implicit operator TIME32(int value) => new(value);
    ...
    public static implicit operator int(TIME32 atomic) => atomic.Value;
    ...
    public static implicit operator TIME32(string value) => Parse(value);
    ...
    public static implicit operator string(TIME32 value) => value.ToString();
