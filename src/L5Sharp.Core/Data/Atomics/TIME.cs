...
/// <summary>
/// Represents a <b>TIME</b> or relative 64-bit time-based value measured in microseconds.
/// This atomic uses a <see cref="long"/> as the underlying primitive in .NET.
/// </summary>
[LogixData(nameof(TIME), true)]
public sealed class TIME : AtomicData, IComparable, IConvertible, IAtomicValue<long>
{
    /// <inheritdoc />
    public TIME(XElement element) : base(element)
    {
    }

    /// <summary>
    /// Creates a new default <see cref="TIME"/> type.
    /// </summary>
    public TIME() : base(nameof(TIME))
    {
    }

    /// <summary>
    /// Creates a new <see cref="TIME"/> with the provided value.
    /// </summary>
    /// <param name="value">The value to initialize the type with.</param>
    public TIME(long value) : this()
    {
    ...
    public int CompareTo(object? obj)
    {
        ...
        {
            null => 1,
            TIME typed => Value.CompareTo(typed.Value),
    ...
    /// <summary>
    /// Parses the specified string representation of a <see cref="TIME"/> value into its corresponding object.
    /// </summary>
    /// <param name="value">The string representation of the <see cref="TIME"/> value to parse.</param>
    /// <returns>A <see cref="TIME"/> object that represents the parsed value.</returns>
    public new static TIME Parse(string value)
    {
        var radix = Radix.Infer(value);
        var typed = radix.Parse<long>(value);
        var formatted = radix.Format(typed);
        return new TIME(CreateDataElement(nameof(TIME), radix, formatted));
    }

    /// <summary>
    /// Attempts to parse the specified string representation of a <see cref="TIME"/> value and returns a value
    /// indicating whether the conversion succeeded.
    /// </summary>
    /// <param name="value">The string representation of the <see cref="TIME"/> value to parse.</param>
    /// <param name="atomic">
    /// When this method returns, contains the <see cref="TIME"/> equivalent of the parsed value if the conversion succeeded;
    /// otherwise, it is null.
    /// </param>
    /// <returns>
    /// <c>true</c> if the value was parsed successfully; otherwise, <c>false</c>.
    /// </returns>
    public static bool TryParse(string? value, out TIME atomic)
    {
        atomic = null!;
        ...
        if (Radix.TryInfer(value, out var radix))
        {
            var typed = radix.Parse<long>(value);
            var formatted = radix.Format(typed);
            atomic = new TIME(CreateDataElement(nameof(TIME), radix, formatted));
            return true;
        }
    ...
    public static implicit operator TIME(long value) => new(value);
    ...
    public static implicit operator long(TIME atomic) => atomic.Value;
    ...
    public static implicit operator TIME(string value) => Parse(value);
    ...
    public static implicit operator string(TIME value) => value.ToString();
