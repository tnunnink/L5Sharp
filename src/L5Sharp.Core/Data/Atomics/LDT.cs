...
/// <summary>
/// Represents a <b>DT</b> or absolute 64-bit Date/Time value measured in nanosecond.
/// This atomic uses a <see cref="long"/> as the underlying primitive in .NET.
/// </summary>
[LogixData(nameof(LDT), true)]
public sealed class LDT : AtomicData, IComparable, IConvertible, IAtomicValue<long>
{
    /// <inheritdoc />
    public LDT(XElement element) : base(element)
    {
    }

    /// <summary>
    /// Creates a new default <see cref="LDT"/> type.
    /// </summary>
    public LDT() : base(nameof(LDT))
    {
    }

    /// <summary>
    /// Creates a new <see cref="LDT"/> with the provided value.
    /// </summary>
    /// <param name="value">The value to initialize the type with.</param>
    public LDT(long value) : this()
    {
    ...
    public int CompareTo(object? obj)
    {
        ...
        {
            null => 1,
            LDT typed => Value.CompareTo(typed.Value),
    ...
    public new static LDT Parse(string value)
    {
        var radix = Radix.Infer(value);
        var typed = radix.Parse<long>(value);
        var formatted = radix.Format(typed);
        return new LDT(CreateDataElement(nameof(LDT), radix, formatted));
    }
    ...
    /// </returns>
    public static bool TryParse(string? value, out LDT atomic)
    {
        atomic = null!;
        ...
        if (Radix.TryInfer(value, out var radix))
        {
            var typed = radix.Parse<long>(value);
            var formatted = radix.Format(typed);
            atomic = new LDT(CreateDataElement(nameof(LDT), radix, formatted));
            return true;
        }
    ...
    public static implicit operator LDT(long value) => new(value);
    ...
    public static implicit operator long(LDT atomic) => atomic.Value;
    ...
    public static implicit operator LDT(string value) => Parse(value);
    ...
    public static implicit operator string(LDT value) => value.ToString();
