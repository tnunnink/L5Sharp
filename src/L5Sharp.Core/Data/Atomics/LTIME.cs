...
/// <summary>
/// Represents a <b>LTIME</b> or relative 64-bit time-based value measured in nanoseconds.
/// This atomic uses a <see cref="long"/> as the underlying primitive in .NET.
/// </summary>
[LogixData(nameof(LTIME), true)]
public sealed class LTIME : AtomicData, IComparable, IConvertible, IAtomicValue<long>
{
    /// <inheritdoc />
    public LTIME(XElement element) : base(element)
    {
    }

    /// <summary>
    /// Creates a new default <see cref="LTIME"/> type.
    /// </summary>
    public LTIME() : base(nameof(LTIME))
    {
    }

    /// <summary>
    /// Creates a new <see cref="LTIME"/> with the provided value.
    /// </summary>
    /// <param name="value">The value to initialize the type with.</param>
    public LTIME(long value) : this()
    {
    ...
    public int CompareTo(object? obj)
    {
        ...
        {
            null => 1,
            LTIME typed => Value.CompareTo(typed.Value),
    ...
    public override bool Equals(object? obj)
    {
        return obj switch
        {
            LTIME value => Value == value.Value,
            AtomicData atomic => Value.Equals((long)Convert.ChangeType(atomic, typeof(long))),
            ValueType value => Value.Equals((long)Convert.ChangeType(value, typeof(long))),
            _ => false
        };
    }
    ...
    /// <summary>
    /// Parses the specified string representation of a <see cref="LTIME"/> value into its corresponding object.
    /// </summary>
    /// <param name="value">The string representation of the <see cref="LTIME"/> value to parse.</param>
    /// <returns>A <see cref="LTIME"/> object that represents the parsed value.</returns>
    public new static LTIME Parse(string value)
    {
        var radix = Radix.Infer(value);
        var typed = radix.Parse<long>(value);
        var formatted = radix.Format(typed);
        return new LTIME(CreateDataElement(nameof(LTIME), radix, formatted));
    }

    /// <summary>
    /// Attempts to parse the specified string representation of a <see cref="LTIME"/> value and returns a value
    /// indicating whether the conversion succeeded.
    /// </summary>
    /// <param name="value">The string representation of the <see cref="LTIME"/> value to parse.</param>
    /// <param name="atomic">
    /// When this method returns, contains the <see cref="LTIME"/> equivalent of the parsed value if the conversion succeeded;
    /// otherwise, it is null.
    /// </param>
    /// <returns>
    /// <c>true</c> if the value was parsed successfully; otherwise, <c>false</c>.
    /// </returns>
    public static bool TryParse(string? value, out LTIME atomic)
    {
        atomic = null!;
        ...
        if (Radix.TryInfer(value, out var radix))
        {
            var typed = radix.Parse<long>(value);
            var formatted = radix.Format(typed);
            atomic = new LTIME(CreateDataElement(nameof(LTIME), radix, formatted));
            return true;
        }
    ...
    public static implicit operator LTIME(long value) => new(value);
    ...
    public static implicit operator long(LTIME atomic) => atomic.Value;
    ...
    public static implicit operator LTIME(string value) => Parse(value);
    ...
    public static implicit operator string(LTIME value) => value.ToString();
