...
/// <summary>
/// Represents a <b>DT</b> or absolute 64-bit Date/Time value measured in microseconds.
/// This atomic uses a <see cref="long"/> as the underlying primitive in .NET.
/// </summary>
[LogixData(nameof(DT), true)]
public sealed class DT : AtomicData, IComparable, IConvertible, IAtomicValue<long>
{
    /// <inheritdoc />
    public DT(XElement element) : base(element)
    {
    }
    ...
    public int CompareTo(object? obj)
    {
        ...
        {
            null => 1,
            DT typed => Value.CompareTo(typed.Value),
    ...
    public new static DT Parse(string value)
    {
        ...
        return new DT(CreateDataElement(nameof(DT), radix, formatted));
    }
    ...
    /// </returns>
    public static bool TryParse(string? value, out DT atomic)
    {
        atomic = null!;
        ...
        if (Radix.TryInfer(value, out var radix))
        {
            var typed = radix.Parse<long>(value);
            var formatted = radix.Format(typed);
            atomic = new DT(CreateDataElement(nameof(DT), radix, formatted));
            return true;
        }
    ...
    public static implicit operator DT(long value) => new(value);
    ...
    public static implicit operator long(DT atomic) => atomic.Value;
