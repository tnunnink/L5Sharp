...
namespace L5Sharp.Core
{
    ...
    public class ModuleCollection : IModuleCollection
    {
        ...
        /// <inheritdoc />
        public IModule? Get(ComponentName name) => _bus?.SingleOrDefault(m => m.Name == name);

        /// <inheritdoc />
        public IModule? Get(string address) => _bus?[address];
        ...
        public IModule? New(ComponentName name, CatalogNumber catalogNumber, byte slot = default,
            ...
            => _bus?.New(name, catalogNumber, slot, ipAddress, description, catalogService);

        /// <inheritdoc />
        public IModule? New(ComponentName name, ModuleDefinition definition, string? description = null) => 
            _bus?.New(name, definition, description);
