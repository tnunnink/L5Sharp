...
namespace L5Sharp.Core
{
    ...
    /// </summary>
    public class ModuleTagCollection : IEnumerable<ITag<IDataType>>
    {
        private readonly List<KeyValuePair<string, ITag<IDataType>>> _tags;

        internal ModuleTagCollection()
        {
            _tags = new List<KeyValuePair<string, ITag<IDataType>>>();
        }
        
        internal ModuleTagCollection(ITag<IDataType>? config, IReadOnlyCollection<Connection> connections) : this()
        {
